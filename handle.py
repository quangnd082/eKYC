...
def get_infor(imag_temp):
    global img, image, ratio, dwdh, ori_images,img_pil
    img = imag_temp
    ...
    try:
        ...
        birth = predict(birth)
        sex  = predict(sex)
        nation = predict(nation)
        address1 = predict(address1)
        address2 = predict(address2)

    # catch error if not found field
    except :
        return None
    

    return id,name, birth, sex, nation, address1, address2


def main(img,origin_img):
    information = get_infor(img)

    if information is not None:
        return information
    else:
        information = get_infor(origin_img)
        if information is not None:
            return information
        else:
